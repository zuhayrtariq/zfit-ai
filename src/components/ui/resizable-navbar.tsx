    ...
    >
      {items.map((item, idx) => (
        <Link
          ...
          onClick={onItemClick}
          className={`relative px-4 py-2  text-neutral-300 ${pathName == item.link && "font-bold"}`}
          key={`link-${idx}`}
          href={item.link}
        >
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <XIcon className="text-primary" onClick={onClick} />
  ) : (
    <MenuIcon className="text-primary" onClick={onClick} />
  );
};
